...
struct EpisodeEditorView: View {
    ...
    private static let triggerOptions = Trigger.allCases.filter { $0 != .unknown }
    ...
    init(_ mode: Mode) {
        ...
        case .edit(let event):
            ...
            _severity = State(initialValue: Double(event.ratedSeverity ?? 5))
            ...
            _selectedTags = State(initialValue: Set(event.triggers.map(\.rawValue)))
            ...
            Form {
                ...
                Section("Triggers") {
                    ForEach(Self.triggerOptions, id: \.self) { trigger in
                        Toggle(trigger.displayName, isOn: setBinding(trigger.rawValue, in: $selectedTags))
                    }
                }
    ...
    private func save() async {
        ...
        switch mode {
        case .create:
            // Derive taxonomy fields from the picked symptom when it's known.
            let lexiconEntry = MockIntelligence.symptomLexicon.first { $0.label == resolvedSymptom }
            var event = HealthEvent(
                timestamp: timestamp,
                symptom: resolvedSymptom,
                category: lexiconEntry?.category ?? .general,
                bodyRegion: lexiconEntry?.region ?? .unspecified,
                severity: Int(severity.rounded()),
                duration: resolvedDuration,
                triggers: Self.triggerOptions.filter { selectedTags.contains($0.rawValue) },
        ...
        case .edit(let event):
            ...
            event.triggersRaw = Self.triggerOptions.filter { selectedTags.contains($0.rawValue) }.map(\.rawValue)
